...
public interface DriverBusAssignmentRepository extends JpaRepository<DriverBusAssignment, Long> {
    ...
    @Query("SELECT a FROM DriverBusAssignment a WHERE a.driver.id = :driverId AND a.unassignedAt IS NULL")
    Optional<DriverBusAssignment> findCurrentByDriverId(@Param("driverId") Long driverId);
